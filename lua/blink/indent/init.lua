...
M.draw = function(winnr, bufnr, force)
  ...
  if bufnr ~= 0 and bufnr ~= vim.api.nvim_get_current_buf() then
    return vim.api.nvim_buf_call(bufnr, function() M.draw(winnr, bufnr, force) end)
  end
  ...
  local indent_levels, whitespace, is_cached = parser.get_indent_levels(bufnr, range)

  -- static
  if config.static.enabled and not is_cached then
    static.draw(winnr, bufnr, config.static.ns, indent_levels, whitespace, range)
